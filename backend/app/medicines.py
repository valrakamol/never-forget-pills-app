...
# -----------------------------------------------------------------------------
# Endpoint สำหรับดึงรายการยาใน Master List (สำหรับ Auto-Complete)
# -----------------------------------------------------------------------------
@medicines_bp.route('/master_list', methods=['GET'])
@jwt_required()
def get_master_medicine_list():
    """
    Endpoint สำหรับดึงรายชื่อยาหลักทั้งหมด
    ส่งกลับเป็น list of objects ที่มี name และ form
    """
    medicines = MasterMedicine.query.order_by(MasterMedicine.name).all()
    
    # --- *** จุดที่แก้ไข: สร้าง list ของ Dictionary *** ---
    medicine_data = [
        {
            "name": med.name,
            "form": med.form 
    ...
    return jsonify(medicines=medicine_data), 200